...
import source from '../postcss-react-native/src/source';
import plugin from '../postcss-react-native/src/index';
...
export default function transformCssModules(conf) {
    ...
    function requireCssFile(filepath, cssFile) {
        return fs.readFileSync(resolveCssFile(filepath, cssFile));
    }

    function resolveCssFile(filepath, cssFile) {
        ...
        return resolve(filepath, filePathOrModuleName);
    }
    ...
    return {
        ...
        visitor: {
            Program(path, {opts}) {
                ...
                const currentConfig = Object.assign({}, defaultOptions, opts);
            ...
            CallExpression(path, {file}) {
                ...
                if (matchExtensions.test(stylesheetPath)) {
                    ...
                    const name = resolveCssFile(requiringFile, stylesheetPath);

                    postcss(plugin({
                        toStyleSheet: function (json, css) {
                            path.replaceWithSourceString(source(json));
                        }
                    })).process(fs.readFileSync(name), {from: name, to: name + '.js'});
